...
#Region "Declarations and Initializations"
    ...
    ' This list will hold all IWAD entries that is available to this program.
    ' NOTE: Property (or {get; set;}) is adjacent to a function call, hence the different naming scheme.
    Public Property DisplayIWADList As New List(Of IWAD)

    ' Exit flag; when the user clicks on either cancel or okay button, this variable will dictate rather
    ' the IWAD list will be updated or remain as-is.
    ' Meaning of the values:
    '   Cancel = False
    '       Nothing is updated
    '   OK = True
    '       Update the source port list
    Public updateIWADList As New Boolean
#End Region
    ...
    Public Sub New(ByVal iwadList As List(Of IWAD))
        ...
        ' Use the incoming IWAD list for possible modification if needed.
        DisplayIWADList = New List(Of IWAD)(iwadList)
    End Sub
    ...
    '  Please see this: https://zdoom.org/wiki/Archive_format
    '  Remarks: "[...] any archive format supported by ZDoom can technically be an IWAD." - https://zdoom.org/wiki/IWAD
    '   > .WAD
    '   > .GRP
    '   > .RFF
    '   > .PAK
    ...
    Private Function GetUserNotes() As String
        ...
        feedback = InputBox(
            "(OPTIONAL) Add notes for this IWAD.  For example: Doom 2: Hell on Earth, Ultimate Doom, or a custom IWAD name",
    ...
    ' For example: "my cat loves to sleep." --> "My cat loves to sleep."
    ' -----------------------
    ' Parameters:
    '   str [String]
    ...
    Private Sub ButtonOK_Click(sender As Object, e As RoutedEventArgs) Handles ButtonOK.Click
        updateIWADList = True       ' Save the IWAD list by overwriting the existing.
    ...
    ' ------------------------------------------
    ' This function will be automatically called when the user clicks an item from the ViewList.
    ' When clicked, this will cache the index that was selected.  We may use that index when the user clicks on the 'Remove' UI button.
    Private Sub ListView_SelectionChanged(sender As Object, e As SelectionChangedEventArgs)
        ' Cache the selected index
        viewListSelectedIndex = ListIWADs.SelectedIndex
    End Sub
End Class
